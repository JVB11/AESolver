...
def compare_mesa_profile(expected_mesa: dict[str, typing.Any],
                         vals_mesa: dict[str, typing.Any]) -> bool:
    ...
    if check_len:
        ...
        for _k, (_v, _t) in expected_mesa.items():
            if _t == np.ndarray:
                ...
                comparison_bool *= np.isclose(my_expect, my_val).all()
            elif _t == float:
                my_expect = float(_v)
